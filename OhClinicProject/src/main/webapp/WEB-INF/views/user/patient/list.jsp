...
<body>
	...
	<div class="content-wrapper">
		<div class="d-flex justify-content-between align-items-center mb-4">
			<h3 class="fw-bold">
				<i class="bi bi-people-fill me-2"></i>환자 리스트 관리
			</h3>
			<div class="btn-group-area">
				<button class="btn btn-outline-secondary me-2"
					onclick="location.href='/ohclinic/adminMain'">
					<i class="bi bi-house-door-fill me-1"></i> 메인 화면
				</button>
				<button class="btn btn-primary"
					onclick="location.href='/ohclinic/patient/createForm'">
					<i class="bi bi-person-plus-fill me-1"></i> 신규 환자 등록
				</button>
			</div>
		</div>

		<div class="search-bar">
			<form action="/ohclinic/patient/search" method="get" class="row g-3">
				<div class="col-md-3">
					...
					<tbody>
						<c:forEach var="p" items="${patientList}">
							<tr>
								...
								<td><span
									class="${p.gender.toString() == 'M' ? 'gender-m' : 'gender-f'}">${p.gender}</span>
									<span class="text-muted">(${p.age}세)</span></td>
								<td>${p.phone}</td>
								<td><small>${p.location}</small></td>
								<td><fmt:formatDate value="${p.lastReserv}"
										pattern="yyyy-MM-dd" /></td>
								<td><c:choose>
										<c:when test="${not empty p.nextReserv}">
											<span class="badge bg-success"><fmt:formatDate
													value="${p.nextReserv}" pattern="yyyy-MM-dd" /></span>
										</c:when>
										<c:otherwise>
								...
								<td><span class="badge bg-light text-dark border">ID:
										${p.doctor.name}</span></td>
								<td class="text-center">
									<button class="btn btn-outline-warning btn-action me-1"
										onclick="confirmUpdate(${p.no})">수정</button>
									<button class="btn btn-outline-danger btn-action"
										onclick="confirmDelete(${p.no})">삭제</button>
								</td>
							</tr>
						</c:forEach>
						...
						<c:if test="${empty patientList}">
							<tr>
								<td colspan="9" class="text-center py-5 text-muted">등록된 환자
									데이터가 없습니다.</td>
							</tr>
						</c:if>
					</tbody>
